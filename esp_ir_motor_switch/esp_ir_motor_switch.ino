...
2. Serail Monitor baud=9600
    ...
    4. /MO<n>OFF   ->  Motor Off <n>   // Added Motor on/off - 02-02-2025
    5. /MO<n>ON   ->  Motor On <n>
   ...
   Other api's
     1. /TANK<n><val>     => set the Tank fill status/percentage n->0-9, val-> 00->99  // Added tank status - 02-02-2025

*************************************************************/
#include <IRrecv.h> // IR lib for esp8266 / 32 =>  IRremoteESP8266 lib
...
String PinSw[ST_DIGITAL]={"D0","D1","D2","D6","D7"};
String MoSw[2]={"Pump-1","Pump-2"};
String PinMo[2]={"S1","S2"};
String MoTank[2]={"Tank-1","Tank-2"};

int st_Digital[ST_DIGITAL]={16,5,4,12,13}; //D0,D1,D2,D6,D7
int mo_digital[2] ={8,9}; //S1,S2
int moval[2]={0,0};
int st_tank[2]={0,0};
String sTank="00";
...
if(WiFi.status() != WL_CONNECTED)rewifi(); // Try reconnect if not connected
 ...
 // Add Motor ON/Off
 if (request.indexOf(MoSw[0]+"ON") > 0)  {  
    Serial.println("On "+MoSw[0]);
      digitalWrite(mo_digital[0], HI);   
       moval[0]=HI; 
    }
  else if (request.indexOf(MoSw[0]+"OFF")>0)  {   
    Serial.println("Off "+MoSw[0]); 
    digitalWrite(mo_digital[0], LO);  
    moval[0]=LO; 
   }
//update Tank status- //GET /TANK<n><val>
if (request.indexOf("/TANK")>0)  {   
     int nidx=0;
      int Sidx=request.indexOf("K")+2; 
      String idx=request.substring(Sidx-1,Sidx);
  String val=request.substring(Sidx,Sidx+2);
  //Serial.println("Tank-"+idx+": "+val); 
     val.trim();
     sTank=val;
     nidx=idx.toInt();
     nidx=nidx%2;  // only max 2 pumps are considered - mo_digital,st_tank,moval
    st_tank[nidx]=val.toInt(); 
   // Serial.println("Tank %"+st_tank[nidx]); 
   if(st_tank[0]>=95){ // Stop the pump
     digitalWrite(mo_digital[nidx], LO);  
    moval[nidx]=LO; 
    }
      if(st_tank[nidx]<=5){ // start the pump
     digitalWrite(mo_digital[nidx], HI);  
    moval[nidx]=HI; 
    }
   }
 ...
 // Serial.println("val="+pattern);
    ...
    if (cmd=="INV")
     {
       if(pattern=="1" || pattern=="H"){ val=1; HI=0;LO=1;sON="On";sOFF="Off"; btMode="Normal"; }
       else if(pattern=="0" || pattern=="L"){ val=0; HI=1;LO=0;sON="Off";sOFF="On";btMode="Inverse"; }
      addr=-1;
 ...
 else if (request.indexOf("/SCN")>0)
 {
  ...
  client.println("<Body>");
  client.println("<H4>Switches-On/Off/Toggel.(Button Mode:"+btMode +")</h4> ");
  for(int j=0;j<ST_DIGITAL;j++){     
   client.println("<br>");
   client.println(OnSw[j]+"("+PinSw[j]+"):<a href=\"/" + OnSw[j] + "ON\"><button>"+sON+"</button></a>");  
 ...
 client.println("<br><hr/>");
  client.println("<H4>Water Pump - On/Off</h4>");
   client.println(MoSw[0]+"("+PinMo[0]+"):<a href=\"/" + MoSw[0] + "ON\"><button>"+sON+"</button></a>");  
   client.println("<a href=\"/"+ MoSw[0] + "OFF\"><button>"+sOFF+"</button></a>");
   if(moval[0]==HI){client.print(sON);}
   else if(moval[0]==LO){client.print(sOFF);}
    client.println("<br>");
    client.println(MoTank[0]+" Status: "+sTank+" % filled." );
...
void init_pins(){  
  ...
  for(int i=0;i<ST_DIGITAL;i++)pinMode(st_Digital[i],OUTPUT);
   ...
   if(inv_val==1){HI=0;LO=1;sON="On";sOFF="Off";btMode="Normal"; }
   else {HI=1;LO=0;sON="Off";sOFF="On";btMode="Inverse"; }
